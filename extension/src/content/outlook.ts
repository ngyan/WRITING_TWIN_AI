/**
 * Outlook Web App content script — injects ✨ Humanize + 🎙 Voice Twin
 * into compose windows on outlook.live.com and outlook.office.com.
 *
...
// ── Selectors ──────────────────────────────────────────────────────────────────
//
// Outlook always gives the compose body role="textbox" + aria-multiline="true".
// We match broadly so locale variants ("Message body", "Corpo do email", etc.) all work.
const COMPOSE_BODY_SEL = [
  'div[role="textbox"][aria-multiline="true"]',
  'div[contenteditable="true"][aria-multiline="true"]',
].join(', ');

// Send button — Outlook surfaces aria-label with optional keyboard-shortcut suffix
// e.g. "Send", "Send (Ctrl+Enter)", "Verzenden (Ctrl+Enter)"
...
function inject(composeBody: Element): void {
  ...
  // Use the send button's closest toolbar, or fall back to its parent
  const toolbar =
    sendBtn.closest('[role="toolbar"]') ??
    sendBtn.closest('[role="group"]') ??
  ...
  async function submitTranscript(): Promise<void> {
    try {
      ...
      writeCompose(composeBody as HTMLElement, resp.result.draft);
      setVoiceStatus('Done ✓');
  ...
  vUndoBtn.addEventListener('click', () => {
    if (originalVoiceText) writeCompose(composeBody as HTMLElement, originalVoiceText);
  ...
  // Insert our host right before the Send button in the toolbar
...
/**
 * Walk up from the compose body to find the Send button.
 * Outlook nests the toolbar in a sibling subtree, so we walk up to a common
 * ancestor (up to 30 levels) and search down for the Send button from there.
 */
...
function writeCompose(el: HTMLElement, text: string): void {
  el.focus();
  // Select all + delete via execCommand (works in Outlook contenteditable)
...
function tryInject(root: Element | Document): void {
  root.querySelectorAll<Element>(COMPOSE_BODY_SEL).forEach((body) => {
    // Skip the global reading pane — we only want active compose windows
...
// MutationObserver — Outlook's React tree mutates deeply
const observer = new MutationObserver(() => tryInject(document));
observer.observe(document.body, { childList: true, subtree: true });

// Polling fallback — catches cases where mutations fire before React finishes rendering
// (e.g. compose window opened but toolbar not yet in DOM when observer fires)
setInterval(() => tryInject(document), 1000);

// Handle compose windows already in DOM on load
tryInject(document);
...
document.addEventListener('keydown', (e) => {
  ...
  if (!toolbarEl) return;
  const hosts = Array.from(toolbarEl.children).filter(c => c.shadowRoot) as HTMLElement[];

  if (e.key === 'H') {
    e.preventDefault();
    (hosts[0]?.shadowRoot?.getElementById('wt-btn') as HTMLButtonElement | null)?.click();
  }
  if (e.key === 'V') {
    e.preventDefault();
    (hosts[0]?.shadowRoot?.getElementById('wt-mic-btn') as HTMLButtonElement | null)?.click();
  }
});
